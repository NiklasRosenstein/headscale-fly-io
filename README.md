...
| `HEADSCALE_NODE_EXPIRY`                          | `180d`                                                         | Default key expiry for non-tagged nodes, regardless of registration method (replaces `oidc.expiry`, which was removed in Headscale 0.29; the legacy `HEADSCALE_OIDC_EXPIRY` variable is still honored as a fallback). Tagged nodes never expire. Setting the value to "0" will mean no expiry.      |
...
| `HEADSCALE_OIDC_EXPIRY`                          | n/a (deprecated)                                               | Deprecated: use `HEADSCALE_NODE_EXPIRY` instead (`oidc.expiry` was removed in Headscale 0.29). Still honored as a fallback for `HEADSCALE_NODE_EXPIRY` for backwards compatibility.                                                                                                                |
| `HEADSCALE_OIDC_USE_EXPIRY_FROM_TOKEN`           | `false`                                                        | Use the expiry from the token received from OpenID when the user logged in, this will typically lead to frequent need to reauthenticate and should only been enabled if you know what you are doing. If enabled, `node.expiry` is ignored.                                                          |
